...
function Navigation(props) {
  return <nav className="w-full p-8 flex flex-row items-center justify-between my-8">
    ...
    <ul className='flex flex-row gap-4 items-center justify-between text-white font-semibold'>
      ...
      <li>Login</li>
      <li>
        <button className="rounded-full bg-blue-700 px-4 py-2">Register</button>
      </li>
    </ul>
  </nav>
}
    ...
    <section className="w-full p-8 py-8 mb-16 flex flex-row items-center content-center justify-center">
      <div className="w-6/12 text-left flex flex-col gap-4">
        <p className='w-fit text-left rounded-full px-2 py-2 bg-white bg-opacity-10'>
          <span className='bg-white rounded-full text-black p-2 mr-2 font-medium'>75% SAVE</span>
          For the black friday weekend.
        </p>
        <h1 className='text-5xl text-left text-white font-extrabold'>Fastest & Secure platform to invest in crypto</h1>
        <p className='w-3/4 text-left'>Buy and sell crypto currencies. Trusted by 10M wallets with over $30 billion in transactions.</p>
        <button className='rounded-full w-fit px-4 py-2 bg-blue-700' >Try for FREE ⚪ </button>
      </div>
  ...
  return (
    <div className='flex flex-row'>
      ...
      <div className='w-16 h-16 inline-block m-0 mr-4 rounded-full bg-white bg-opacity-10' >
        <img className='h-full w-full p-4 object-center align-middle ' src={image} alt="" />
      </div>
  ...
  return (
    <div className='p-6 bg-white rounded-lg justify-between absolute top-28 z-10 text-left'>
      <div className='w-full flex flex-row gap-4 mb-12 text-left'>
        <input className='w-56 bg-transparent placeholder:text-black border-b border-b-black text-left' type="text" name="hash_rate" id="hash_rate_input" placeholder='Enter you hash rate' />
        <select className='w-32 bg-transparent text-black border-b border-b-black text-left ' name="hash_units" id="hash_units_input">
          <option value="TH/s">TH/s</option>
          <option value="GH/s">GH/s</option>
          <option value="MH/s">MH/s</option>
        </select>
        <button className='bg-blue-600 text-white p-2 rounded-full'>Calculate</button>
      </div>
      <div className='text-left text-black '>
        <p className='text-blue-600 mb-4'>ESTIMATED 24 HOUR REVENUE:</p>
        <h3 className='font-medium text-2xl'>0.005 130 50 ETH<span className='text-blue-500'>{`($1275)`}</span></h3>
        <small className='text-sm text-opacity-40'>Revenue will change base on mining difficulty and Etherium price.</small>
      </div>
    </div>
  )
}
  ...
  return (
    <section className='w-full p-8 mb-16 flex flex-col gap-16 text-left'>
      <div className='w-full flex flex-row items-center justify-between'>
        <ShowCase key={1} image={I_1} highlight={"$30B"} desc={"Digital Curerncy Exchanged"} />
        <ShowCase key={2} image={I_2} highlight={"10M+"} desc={"Trusted wallet investors"} />
        <ShowCase key={3} image={I_3} highlight={"195"} desc={"Countries supported"} />
      </div>
      ...
      <div className='w-full flex flex-row items-center justify-between gap-8'>
        ...
        <div className='flex-1 text-left flex flex-col gap-4 justify-between'>
          ...
          <p className="text-left">
            Expreience the next generation crypto currency platform. No finance boudries, extra fees, and fake reviews.
          </p>
          <button className='w-fit px-4 py-2 rounded-full bg-blue-700'>Learn more</button>
        </div>
      </div>
...
function NavCard({ image, title, subTitle, desc, link, linkText }) {
  return (
    <div className='group p-8 flex flex-col gap-4 w-64 items-center justify-between bg-white text-black rounded-md hover:bg-indigo-900 hover:text-white text-center'>
      ...
      <a className='text-white bg-blue-500 px-4 py-2 rounded-full items-center align-middle hover flex flex-row' href={link} >
        <span className=''>
          {linkText}
        </span>
        <span className='ml-2 bg-white p-1 rounded-full leading-none w-5 align-middle text-blue-600 text-xs text-center font-extrabold'>{">"}</span>
      </a>
    </div>
  )
  ...
  return (
    <section className='w-full relative bg-blue-100 p-8 mb-16 pt-36 py-16 top-20'>
      ...
      <div className='w-full flex flex-row items-center justify-center gap-16'>
        <NavCard image={BcI} title={`Bitcoin`} subTitle={`BTC`} desc={`Digital currency in which a record of transactions is maintained.`} link={`#`} linkText={`Start Mining`} />
        <NavCard image={EcI} title={`Litecoin`} subTitle={`LTC`} desc={`Digital currency in which a record of transactions is maintained.`} link={`#`} linkText={`Start Mining`} />
        <NavCard image={LcI} title={`Etherium`} subTitle={`ETC`} desc={`Digital currency in which a record of transactions is maintained.`} link={`#`} linkText={`Start Mining`} />
      </div>
    </section>
  )
}
...
function Section_4() {
  return (
    <section className='py-16 pt-32 p-8 mb-16'>
      <h2 className='w-1/2 mx-auto text-center font-bold text-4xl mb-12'>Market sentiments, portfolio, and run the infrastructure of your choice</h2>
      <div className='w-full mx-auto flex flex-col gap-24'>
        ...
        <div className='w-full mx-auto flex flex-row gap-12'>
          <div className='w-5/12 text-left flex flex-col gap-4'>
            ...
            <a className='py-3 px-4 leading-none font-semibold rounded-full text-white bg-blue-600 w-fit' href="">Learn more</a>
          </div>
        ...
        <div className='w-full flex flex-row items-center justify-between bg-blue-400 p-8 rounded-lg gap-8'>
          <div className='text-left w-1/2'>
            <h3 className='text-left text-2xl font-bold mb-2' >Start mining now</h3>
            <p className='text-left text-base'>Join now with CRAPPO to get the latest news and start mining now</p>
          </div>
          <form className=''>
            <input className='w-64 inline bg-transparent border-b border-b-black placeholder:text-white mr-4' type="email" name="" id="" placeholder='Enter your email' />
            <input className='inline bg-white rounded-full px-4 py-3 leading-none text-black' type="submit" value="Subscribe" />
          </form>
        </div>
  ...
  return (
    <footer className="w-full p-8 flex flex-col items-center justify-start gap-20 pb-8">
      <div className="w-full flex flex-row items-start justify-between">
        ...
        <div className="flex-1 ">
          <h3 className="text-2xl font-bold text-left mb-8">We accept following payment systems</h3>
